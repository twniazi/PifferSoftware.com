                            ...
                            <tr>
                                ...
                                <td style="display:flex; gap: 10px; align-items: center;">
                                    <form action="{{ route('viewhrm', ['id' => $hrm->id]) }}" >
                                        @csrf
                                        <button type="submit" class="btn" style="border-radius:20%;"><i class="material-icons" style="color: rgb(92, 92, 255);">visibility</i></button>
                                    </form>
                                    @if (Auth::user()->role != 'customer' && Auth::user()->role != 'client')
                                    <form action="{{ route('edithrm', ['id' => $hrm->id]) }}" >
                                        @csrf
                                        <button type="submit" class="btn" style="border-radius:20%;"><i class="material-icons" style="color: rgb(57, 221, 57);">edit</i></button>
                                    </form>
                                    @endif
                                </td>
                            </tr>
  ...
  $('#hrm-search').on('input', function() {
    ...
    $.ajax({
        ...
        error: function (xhr, status, error) {
            ...
            $('#employee-table-body').html('<tr><td colspan="6">There was an error processing your request.</td></tr>');
        }
    });
});
