...
@php
    ...
    <tbody>
        @foreach ($feedbackreport as $index => $dailyReports)
    @php
        $date = $index; // since the key is the date
    @endphp
    <tr>
        ...
        @foreach ($cros as $cro)
            @php
                $report = $dailyReports->firstWhere('cro_id', $cro->id);
            @endphp
            <td>
                @if ($report && $report->attachment)
                    <a href="{{ asset($report->attachment) }}" target="_blank" class="bg-success text-white text-decoration-none p-1 rounded"><strong>Received</strong></a>
                @else
                    <strong class="bg-danger text-white text-decoration-none p-1 rounded">Not Received</strong>
                @endif
            </td>
        @endforeach
    </tr>
        ...
        $('#analyticsTable').DataTable({
            ...
            buttons: [
                ...
                'pdfHtml5',
                {
                    ...
                    messageTop: 'Generated Report - Feedback Register Report',
